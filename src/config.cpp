...
namespace Config {
    ...
    {
        if (!ReadFloat("upOffset", options.upOffset)) return false;
        if (!ReadFloat("rightOffset", options.rightOffset)) return false;
        if (!ReadFloat("upMultiplier", options.upMultiplier)) return false;
        if (!ReadFloat("rightMultiplier", options.rightMultiplier)) return false;
    ...
    const std::string & GetConfigPath()
    {
        ...
        if (s_configPath.empty()) {
            ...
            if (!runtimePath.empty()) {
                s_configPath = runtimePath + "Data\\SKSE\\Plugins\\console_selection_fix.ini";
